...
def test_jack_match_starter_suit_scores_one():
    hand = build_hand(["ah","ac","jh","ks"])
    assert JackMatchStarterSuitScorer().check(hand, starter=Card("2h")) == (1, "Jack match starter suit")

def test_jack_match_starter_suit_scores_zero():
    hand = build_hand(["ah","ac","jh","ks"])
    assert JackMatchStarterSuitScorer().check(hand,starter=Card("2d")) == (0, "")

def test_no_jack_in_hand_and_starter_is_jack():
    hand = build_hand(["ah","ac","2h","ks"])
    assert JackMatchStarterSuitScorer().check(hand, starter=Card("jh")) == (0, "")